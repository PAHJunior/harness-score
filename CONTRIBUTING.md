...
Open an issue before a large PR (a new check, a check change) so we can
agree on the approach first — check changes touch multiple files that must
...
3. **The maturity model lives in three places that must change together:**
...
1. Open an issue using the **[Check change](https://github.com/paladini/harness-score/issues/new?template=check_change.yml)**
...
3. Update `docs/guide/maturity-model.md` and
   ...
   and `packages/cli/test/maturity-sync.test.ts` enforce that check IDs,
